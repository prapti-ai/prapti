...
def test_logger_smoke(caplog):
    ...
    log.critical("lots-of-errors", f"lots too many errors. an error message.", fake_source_loc1)
    ...
    log.info("info-1", f"info message")
    log.detail("detail-1", f"detail with message id")
    log.detail(f"detail without message id")
    log.debug(f"debug dump without message id")
